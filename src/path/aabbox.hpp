...
struct AABBox {
  glm::dvec3 bl; // Bottom left (min)
  glm::dvec3 tr; // Top right   (max)

  AABBox(glm::dvec3 bl_ = glm::dvec3(), glm::dvec3 tr_ = glm::dvec3()) {
    bl = bl_, tr = tr_;
  }
  ...
  // Expand to fit point
  void expand(const glm::dvec3& vec) {
    if (vec.x < bl.x)
      bl.x = vec.x;
    if (vec.y < bl.y)
      bl.y = vec.y;
  ...
  // Check if ray intersects with box. Returns true/false and stores distance in
  // t
